...
#define NR_AVAIL_NODES (7)			// only for 4 nodes experiment
...
int main(int argc, char ** argv)
{
	...
	if (argc > 1 && argc <= 8){
		if(0 <= atoi(argv[1]) && atoi(argv[1]) < NR_NODES &&
			...
			0 <= atoi(argv[4]) && atoi(argv[4]) < NR_NODES && 
			0 <= atoi(argv[5]) && atoi(argv[5]) < NR_NODES && 
			0 <= atoi(argv[6]) && atoi(argv[6]) < NR_NODES && 
			0 <= atoi(argv[7]) && atoi(argv[7]) < NR_NODES ){
			nodemask_set_compat(&nodelist, atoi(argv[1]));
			nodemask_set_compat(&nodelist, atoi(argv[2]));
			nodemask_set_compat(&nodelist, atoi(argv[3]));
			nodemask_set_compat(&nodelist, atoi(argv[4]));
			nodemask_set_compat(&nodelist, atoi(argv[5]));
			nodemask_set_compat(&nodelist, atoi(argv[6]));
			nodemask_set_compat(&nodelist, atoi(argv[7]));
		}else{
