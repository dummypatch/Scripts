...
do
	...
	do
		...
		do
			...
			TWO_HOP_PATH=`echo ".*BW(${LIST_NODE_ID[${src_idx}]}->${LIST_NODE_ID[${mid_idx}]}->${LIST_NODE_ID[${dest_idx}]}).*"`
			COUNT=0
			COUNT=`cat ${FILENAME} | grep ${TWO_HOP_PATH}|wc -l`
			cat ${FILENAME} | grep ${TWO_HOP_PATH} > ${TEMP_FILENAME}
			if [ ${COUNT} -gt 0   ] ;then
				for (( line_idx=1;line_idx <= ${COUNT} ; line_idx++))
				do
